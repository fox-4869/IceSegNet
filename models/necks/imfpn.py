...
class IMFPN(BaseModule):
    def __init__(self, in_channels, out_channels, num_outs, start_level=0, end_level=-1,
        ...
        self.attention_convs = nn.ModuleList()  # Attention layers
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            attn_conv = ConvModule(
                ...
                norm_cfg=norm_cfg,
                act_cfg=act_cfg)
            self.attention_convs.append(attn_conv)
    ...
    def forward(self, inputs):
        ...
        # Apply attention
        laterals = [self.attention_convs[i](laterals[i]) for i in range(len(laterals))]
